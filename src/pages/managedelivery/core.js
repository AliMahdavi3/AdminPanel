...
import { addNewDeliveryService, updateDeliveryServices } from "../../services/delivery";
...
export const onSubmit = async (values, actions, setData, deliveryToEdit) => {
    if (deliveryToEdit) {
        const res = await updateDeliveryServices(deliveryToEdit.id, values)
        if (res.status == 200) {
...
.shape({
    ...
    time_unit: Yup.string().required("لطفا این قسمت را پر کنید"),
});